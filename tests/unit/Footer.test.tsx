...
describe("Footer — unauthenticated", () => {
  beforeEach(() => {
    mockUseAuth.mockReturnValue({ isAuthenticated: false, isVenueManager: false });
    render(<Footer />);
  });
  ...
  it("renders Browse venues link", () => {
    expect(screen.getByRole("link", { name: "Browse venues" })).toBeInTheDocument();
  });
...
describe("Footer — authenticated customer (not venue manager)", () => {
  beforeEach(() => {
    mockUseAuth.mockReturnValue({ isAuthenticated: true, isVenueManager: false });
    render(<Footer />);
  });
...
describe("Footer — authenticated venue manager", () => {
  beforeEach(() => {
    mockUseAuth.mockReturnValue({ isAuthenticated: true, isVenueManager: true });
    render(<Footer />);
  });
